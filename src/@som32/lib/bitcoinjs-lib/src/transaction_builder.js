...
TransactionBuilder.prototype.signHasSig = function (vin, sigPair, redeemScript, hashType, witnessValue, witnessScript) {
  // TODO: remove sigPair.network matching in 4.0.0
  if (sigPair.network && sigPair.network !== this.network) throw new TypeError('Inconsistent network')
  if (!this.__inputs[vin]) throw new Error('No input at index: ' + vin)
  hashType = hashType || Transaction.SIGHASH_ALL
  ...
  const ourPubKey = sigPair.publicKey
  if (!canSign(input)) {
    if (witnessValue !== undefined) {
      if (input.value !== undefined && input.value !== witnessValue) throw new Error('Input didn\'t match witnessValue')
      typeforce(types.Satoshi, witnessValue)
